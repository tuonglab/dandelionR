...
To begin, we will filter the data to remove cells with no paired chains and extract the TCR information so that we can construct pseudobulks. 
```{r, warning = FALSE}
...
The next step is to project the pseudotime information from the pseudobulks back to each cell in the dataset. This is done by computing the distance of each cell to the pseudobulked VDJ feature space and projecting the pseudotime information to the nearest neighbour in the pseudobulked VDJ feature space.
