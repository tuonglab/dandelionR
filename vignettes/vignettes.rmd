...
pca <- t(as.matrix(reducedDim(pb.milo, type = "PCA")))  # extract the PCA matrix
branch.tips <- c(which.max(pca[2,]),which.min(pca[2,])) # define the CD8 terminal cell as the top-most cell and CD4 terminal cell as the bottom-most cell
names(branch.tips)<-c("CD8+T","CD4+T")
root <- which.min(pca[1,]) # define the start of our trajectory as the left-most cell
```
...
pb.milo <- markov_probability(pb.milo, dm, branch.tips, root)
```
