...
#' @export
project_pseudotime_to_cell <- function(
    adata,
    pb_adata,
    term_state,
    suffix = ""
  ...
  #pb_assign_trim = nhood[nhoodsum >0]?????
  
  # for each cell pesudotime_mean is the average of the pseudobulks the cell is in, weighted by 1/ neighbourhood size
  ...
  new_col <- vapply(colData(pb_adata)[,col_list]@listData , function(x,y){ list(as.vector(x%*%y / apply(y,2,sum)))}, y = nhoods_cdata_norm, FUN.VALUE = list(double()))
