...
#'
#' @export
setupVdjPseudobulk <- function(sce, mode_option = c("abT", "gdT", "B"),
    already.productive = TRUE, productive_cols = NULL, productive_vj = TRUE,
    productive_vdj = TRUE, allowed_chain_status = NULL, subsetby = NULL,
    groups = NULL, extract_cols = NULL, filter_unmapped = TRUE, check_vj_mapping = c(TRUE,
        TRUE), check_vdj_mapping = c(TRUE, FALSE, TRUE), check_extract_cols_mapping = NULL,
    remove_missing = TRUE) {
    ...
    if (length(check_vj_mapping) != 2) {
        abort(paste("ValueError: length of check_vj_mapping should be 2. But",
            length(check_vj_mapping), "was provided."))  # nocov
    }
    .typeCheck(check_vdj_mapping, "logical")
    if (length(check_vdj_mapping) != 3) {
        abort(paste("ValueError: length of check_vj_mapping should be 3. But",
            length(check_vdj_mapping), "was provided."))  # nocov
    }
    ...
    if (!already.productive) {
        sce <- .filterProductivity(sce, mode_option, productive_cols, productive_vj,
            productive_vdj)
    }
    ...
    if (filter_unmapped) {
        sce <- .filterUnmapped(sce$sce, mode_option, check_vj_mapping,
            check_vdj_mapping, main_cols = sce$main_cols, check_extract_cols_mapping,
            remove_missing)
    } else {
        sce <- sce$sce
    }
...
.filterProductivity <- function(sce, mode_option, productive_cols, productive_vj,
    productive_vdj) {
    # check the input
    if (is.null(mode_option)) {
        if (!is.null(productive_cols)) {
            ...
            sce <- Reduce(function(data, p_col) {
                idx <- substr(colData(data)[[p_col]], start = 1, stop = 1) ==
                  "T"
                data[, idx]
        ...
        if (is.null(productive_cols)) {
            produ_col <- paste("productive", mode_option, c("VDJ", "VJ"),
                sep = "_")[c(productive_vdj, productive_vj)]
        } else {
...
.extractVdj <- function(sce, extract_cols, mode_option) {
    ...
    if (!length(grep("_VDJ_main|_VJ_main", names(colData(sce))))) {
        ...
        main_cols <- colnames(colData(sce))[grep("_VDJ_main|_VJ_main",
            names(colData(sce)))]
...
.generateExtractColumn <- function(sce, extract_cols) {
    ...
    if (!any(extract_cols %in% colnames(colData(sce)))) {
        message(sprintf("ColData does not exist, Creating %s colData based on column CTgene",
            msg))
        ...
        if (length(splitVdj[[1]]) != length(extract_cols)) {
            abort(paste("Keyerror: Colnames", paste0(extract_cols[!extract_cols %in%
...
#' @importFrom rlang abort
#' @return filtered SingleCellExperiment object
.filterUnmapped <- function(sce, mode_option, check_vj_mapping, check_vdj_mapping,
    main_cols, check_extract_cols_mapping, remove_missing) {
    ...
    if (!is.null(extr_cols)) {
        ...
        sce <- Reduce(function(x, y) {
            .filterCells(sce = x, col_n = y, filter_pattern = filter_pattern,
