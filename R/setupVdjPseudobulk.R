...
setupVdjPseudobulk <- function(sce, mode_option = c("abT", "gdT", "B"),
    ...
    if (!already.productive) {
        sce <- .filterProductivity(sce, mode_option, productive_cols,
                                   productive_vj, productive_vdj)
    }
    ...
    if (filter_unmapped) {
        sce <- .filterUnmapped(sce$sce, mode_option, check_vj_mapping,
                               check_vdj_mapping, main_cols = sce$main_cols,
    ...
    if (is.null(mode_option)) {
        if (!is.null(productive_cols)) {
            ...
            sce <- Reduce(function(data, p_col) {
                idx <- substr(colData(data)[[p_col]], start = 1,
                              stop = 1) == "T"
                data[, idx]
    ...
    if (!is.null(allowed_chain_status)) {
        ...
        if (!any(idx)) {
            ...
            current_cs <- paste(unique(colData(sce)[["chain_status"]]),
                                collapse = ", ")
            abort(sprintf(paste0("Unsuitable allowed_chain_status,\n ",
...
.extractVdj <- function(sce, extract_cols, mode_option) {
    ...
    if (!length(grep("_VDJ_main|_VJ_main", names(colData(sce))))) {
        ...
        main_cols <- colnames(colData(sce))[grep("_VDJ_main|_VJ_main",
                                                 names(colData(sce)))]
        warning("main VDJ information already exists,",
...
.generateExtractColumn <- function(sce, extract_cols) {
    ...
    if (!any(extract_cols %in% colnames(colData(sce)))) {
        ...
        abort(sprintf(paste0("Keyerror: Colnames %s do not exist in colData\n ",
...
.filterUnmapped <- function(sce, mode_option, check_vj_mapping,
    ...
    if (length(check_vj_mapping) != 2) {
        abort(sprintf(paste0("ValueError: length of check_vj_mapping should be",
                             " 2. But %d was provided."),
                      length(check_vj_mapping)))  # nocov
    }
    .typeCheck(check_vdj_mapping, "logical")
    if (length(check_vdj_mapping) != 3) {
        abort(sprintf(paste0("ValueError: length of check_vj_mapping should be",
        " 3. But %d was provided."), length(check_vdj_mapping)))  # nocov
    }
