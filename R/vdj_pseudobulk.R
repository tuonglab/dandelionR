...
{ 
  ...
  if(is(milo, "Milo")) pbs = milo@nhoods
  else pbs = .get.pbs(pbs, obs_to_bulk, milo)
  ...
  if(is.null(extract_cols))
  {
    ...
    {
      all.col.n<-colnames(colData(milo))
      extract_cols <-  all.col.n[grep(paste0(mode_option,"_VDJ_main|",mode_option,"_call_VJ_main"),all.col.n)]
    }
  }
  
  # perform matrix multiplication of pseudobulks by cell matrix by a cells by VJs matrix
  # strat off by creating the cell by VJs matix
  ...
  vjs0[] <- lapply(vjs0, function(x) if(!is.factor(x)) as.factor(x) else x)
  one_hot_encoded <- model.matrix(~ . - 1 , data = vjs0, contrasts.arg = lapply(vjs0, contrasts, contrasts = FALSE)) # prevent reference level
  ...
  if(normalise)
  {
    ...
    for (col_n in extract_cols)
    {
      ...
      # compute sum of of non-missing values for each pseudobulk for this category
      ...
      if(renormalise)
      {
        redefine.count <- apply(pseudo_vdj_feature[defined.min.counts,group.define.mask],1,sum)
        pseudo_vdj_feature[defined.min.counts,group.define.mask] <- pseudo_vdj_feature[defined.min.counts,group.define.mask]/define.count
      }
      pseudo_vdj_feature[!defined.min.counts,group.define.mask]<-0
    }
  }
  # create obs for the new pesudobulk object
  #milo@metadata$feature.space <- pseudo_vdj_feature
  ...
  pb.sce <- SingleCellExperiment(assay = SimpleList( X = t(pseudo_vdj_feature)),
                                   ...
                                   colData = pbs.obs)
  # store pseudobulk assignment, as a sparse for storage efficiency
