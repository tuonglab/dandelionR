...
.construct.markov.chain <- function(wp_data, knn., pseudotime, waypoints)
{
  ...
  ind <- lapply(idx_seq, function(x){KNN@i[x]+1})
  ...
  ## Remove edges that move backwards in pseudotime except for edges that are within the computed standard deviation
  rem_edges <- pmap(list(.x = traj_nbrs ,.y = (pseudotime - adaptive.std),  .z = ind), function(.x,.y,.z){
    .z[.x < .y]
  })
  for(i in 1:length(waypoints))
  {
    if(length(KNN[i,rem_edges[[i]]])) KNN[i,rem_edges[[i]]] <- 0
  }
  ...
  aff<-exp(
    -(ids$x**2) / (adaptive.std[ids$i]**2) * 0.5
  ...
  D = apply(W,1,sum)
  ids = summary(W)
  T_ = sparseMatrix(i = ids$i, j = ids$j, x = ids$x / D[ids$i], dims = dim(KNN), giveCsparse = TRUE)
  return(T_)
}
