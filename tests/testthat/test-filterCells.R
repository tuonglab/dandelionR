...
col_data <- data.frame(
    VDJ = c("None", "TRBV7-9", "TRBV7-9", "None", "TRBV7-9", "No_contig", "TRBV7-9", "TRBV7-9", "None", "TRBV7-9")
)
...
test_that(".filterCells works correctly with remove_missing = FALSE", {
    ...
    expect_true(all(colData(result)$VDJ[grep("None|No_contig", colData(result)$VDJ)] == "VDJ_missing"))
})

# Additional tests for edge cases
test_that(".filterCells handles edge cases", {
    # Test with empty SingleCellExperiment
    empty_sce <- SingleCellExperiment(assays = list(counts = matrix(nrow = 0, ncol = 0)))
    expect_error(.filterCells(empty_sce, "VDJ"), "None column remains, please check whether the filtering option is correct.")

    # Test with no matching filter pattern
    no_match_sce <- SingleCellExperiment(assays = list(counts = data), colData = data.frame(VDJ = rep("TRBV7-9", 10)))
