...
test_that(".classCheck works correctly", {
    ...
    expect_error(.classCheck(123, "character"), "The '123' must be character, not numeric")
    expect_error(.classCheck(TRUE, "logical"), NA)
    expect_error(.classCheck("test", "logical"), "The 'test' must be logical, not character")
})
...
test_that(".typeCheck works correctly", {
    ...
    expect_error(.typeCheck(123, "character"), "The '123' must be character, not double")
    expect_error(.typeCheck(TRUE, "logical"), NA)
    expect_error(.typeCheck("test", "logical"), "The 'test' must be logical, not character")
})
